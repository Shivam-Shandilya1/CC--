...
public:
    ...
    using Type = AStar<Graph, Debug>;

    using HeurisitcFunction = std::function<int(Vertex)>;
public:
...
public:
    AStar(const GRAPH& graph, const std::vector<int>& weight, const HeuristicFunction& heuristicFunc) :
        graph(graph),
        weight(weight),
        Q(graph.numVertices()),
    ...
    AStar(const GRAPH& graph) :
        AStar(graph, graph[TravelTime]){
    }

    template<AttributeNameType ATTRIBUTE_NAME>
    AStar(const GRAPH& graph, const AttributeNameWrapper<ATTRIBUTE_NAME> weight) :
        AStar(graph, graph[weight]){
    }

    AStar(const GRAPH&&, const std::vector<int>&) = delete;
    AStar(const GRAPH&, const std::vector<int>&&) = delete;
    AStar(const GRAPH&&) = delete;

    template<AttributeNameType ATTRIBUTE_NAME>
    AStar(const GRAPH&&, const AttributeNameWrapper<ATTRIBUTE_NAME>) = delete;
    ...
    inline void addSource(const Vertex source, const int distance = 0) noexcept {
        ...
        sourceLabel.heuristic = heuristicFunc(source);
        ...
        while(!Q.empty()) {
            ...
            for (const Edge edge : graph.edgesFrom(u)) {
                ...
                if (vLabel.distance > distance) {
                    ...
                    vLabel.parent = u;
                    vLabel.heuristic = heuristicFunc(v);  // Apply the heuristic
                    Q.update(&vLabel);
                }
            }
            settle(u);
