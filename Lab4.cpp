...
numberOfItems++;
 ...
 void simulate(int arrivingRate, int leavingRate, int totalTime)
 {
     Queue<int> q;
     int j = 0,cnt = 0;
     while(totalTime>0) {
         int i = 0;
         while(i<arrivingRate && totalTime>0)
         {
             while(j<leavingRate && i<arrivingRate){
                 q.enqueue(cnt);
                 j++;
                 i++;
             }
             
             if(j==leavingRate || leavingRate>=arrivingRate)
             {
             while(j>0)
             {
                 cout<<q.dequeue()<<" ";
