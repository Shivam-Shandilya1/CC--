...
int match(State *start, char *s)
{
 ...
 void findValidTokens(char *r, char*w) {
char * postfixExpression = regexToPostfix(r);
State *start = post2nfa(postfixExpression);
l1.s = (State**) malloc(numberOfStates * sizeof l1.s[0]);
   ...
   while (strlen(w) > 0) {
     bool matched = false;
     for (int i = strlen(w); i >= 0; i--) {
       string sub = w.substr(0, i);
       if (//regex_match(sub, reg)
       match(start,w )
 ...
 int main() {
   char r[1000]  = "(((a)(((((a)((b)*))(a))|(((b)((a)*))(b)))*))(a))";
   char w[1000] = "aaaabbbbaaabbb";
    // int i;
