...
int match_nfa(string s,State *head_node, State *ghost_state, int ghost_int,Path path_l1, Path path_l2)
{
    State *tmp = ghost_state,*tmp2;
    int k;
...
int main() {
    ...
    int num_regex_fn;
    string num_Regex_fn_str;
    getline(in,num_Regex_fn_str);
    num_regex_fn = stoi(num_Regex_fn_str);
    ...
    vector<string> regex_list(num_regex_fn);
    string InputString;
