...
// Thompson's construction algorithm is a popular method for building a Non-Deterministic Finite Automaton (NFA) from a regular expression. The algorithm works by defining a set of small building blocks called "fragments", which can be combined using regular expression operators to construct larger fragments that correspond to more complex regular expressions.

// In this implementation, each fragment is represented by a "Fr" struct, which contains three pointers:

// "head_node": A pointer to the first state of the fragment
// "out": A pointer to the "exit" state of the fragment
// "tail_node": A pointer to the last state of the fragment
// The "head_node" is used to track the first state of the fragment, while the "tail_node" is used to track the last state of the fragment. The "out" pointer represents the "exit" point of the fragment, and is used to link the fragment to other fragments that it is combined with.

// The algorithm starts by iterating through the regular expression from left to right. At each step, it processes the next character in the expression and performs an operation based on the type of the character:

// If the character is a regular expression symbol (such as "+", "*", ".", "?", or "|"), the algorithm pops one or more fragments from a stack, combines them using the appropriate operator, and pushes the resulting fragment back onto the stack.

// If the character is a regular character (such as "a", "b", "c", etc.), the algorithm creates a new fragment consisting of a single state that accepts the character, and pushes it onto the stack.

// Once the entire regular expression has been processed, the final fragment on the stack represents the NFA that corresponds to the regular expression.

// To build the NFA, the algorithm combines the fragments on the stack by linking their "out" pointers to the "head_node" of the next fragment. The "out" pointer of the final fragment is linked to a special "matchstate" that indicates a successful match.

// The resulting NFA can then be used to recognize strings that match the regular expression. When a string is input to the NFA, it starts at the "start state" and transitions through the NFA based on the input characters. If the NFA reaches the "matchstate", the input string is considered a match.

// In summary, fragments are a key concept in Thompson's construction algorithm for building an NFA from a regular expression. Each fragment represents a small building block that can be combined with other fragments using regular expression operators to form larger fragments. The use of fragments simplifies the implementation of the algorithm and allows for an efficient construction of the resulting NFA.

State* PostFixExpressionToNFA(string postfix_expression)
{
    State *st;
...
// Here's how the regular expression would be converted to postfix notation using the given function:

// Start with an empty stack and an empty output string.

// Iterate through the characters of the regular expression from left to right.

// When you encounter an opening parenthesis '(', push it onto the stack.

// When you encounter a closing parenthesis ')', pop operators off the stack and append them to the output string until you reach the matching opening parenthesis. Discard both the opening and closing parentheses.

// When you encounter an operator such as '*', '+', or '?', append it to the output string.

// When you encounter a character, append it to the output string.

// When you encounter a pipe symbol '|', pop operators off the stack and append them to the output string until you reach an opening parenthesis or the end of the stack. Then push the pipe symbol onto the stack.

string RegularExpressionToPostFix(string regularExpression) 
{
    //Creating Static Character Buffer_matrixfer of length 8k
    static char buffer_matrix[8*1000];//TO STORE POSTFIX EXPRESSION
    ...
    int numberOfAlternations=0; // number of OR operator
    int numOfAtoms=0; // smallest unit of pattern that can be matched
    ...
    for(auto it = regularExpression.begin(); it!=regularExpression.end(); it++)
    {
        ...
        switch(*it)
        {
            ...
            case ')':
                // tmp_int ++;
                if((ptr == expression_Stack || !numOfAtoms))
                {
                    // tmp2 = NULL;
                    return "";
                }
