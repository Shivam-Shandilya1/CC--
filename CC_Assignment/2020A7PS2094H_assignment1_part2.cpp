...
int regex_match(string regex,string inputString,State *ghost_State, int ghost_int)
{
    // cout << regex <<endl;
    // cout << InputString<<endl;

    State *tmp_state = ghost_State;
    ...
    int res;
    if(tmp_int2)res = match_nfa(inputString,head_node,NULL,0);
...
string printResult(string InputString, int ghost_int, vector<string> regexListarr, int regexListCnt) {
    ...
    while (InputString.size()) 
    {
        ...
        vector<int> regexListIndex(regexListCnt);

        for (int i = 0; i < regexListCnt; i++)
        {
            string regularExpression = regexListarr[i];
            
            bool innerLoopflag = false;
            ...
            vector<string> matchList_arr;
            
            tmp3 = tmp2;

            tmp2 = 1;

            for(int j=0; j<n && tmp2; j++)
            {
                string substring=InputString.substr(0, j + 1);

                if(tmp2 && regex_match(regularExpression,substring,NULL,0))
                {
                    innerLoopflag = true;

                    tmp2++;

                    matchList_arr.push_back(substring);

                    matchListCnt++;
                }
            }

            if(tmp2 && matchListCnt != 0 && !(matchList_arr.back().size()))
            {
                tmp2 = 1;
            ...
            if(tmp2 && innerLoopflag)
            {
                outerLoopFlag = true;
                tmp2++;
                tmp3 = tmp2;
                int position = matchList_arr.back().size();
                tmp1 = tmp2;
                tmp2 = 1;
                regexListIndex[i] = position;
            }else
            {
                tmp2 = 1;
                tmp2++;
        ...
        if (tmp2 && outerLoopFlag)
        {
            int MaxMatchedSubstring = 0;

            tmp2++;
            
            int tmp3 = tmp2;

            for (int i =0; i<regexListCnt && tmp2; i++)
            {
                tmp2++;

                if(tmp2 && regexListIndex[i]>MaxMatchedSubstring)
                {
                    tmp3 = tmp2;
                    MaxMatchedSubstring = regexListIndex[i];
                    tmp2++;
                    tmp1 = tmp2;
                }
            }
            ...
            for(int i = 0;tmp2 && i<regexListCnt; i++)
            {
                tmp3 = tmp2;
                tmp2++;
                for(int l = 2;l<4;l++)tmp2++;
                ...
                if(tmp2 && regexListIndex[i] == MaxMatchedSubstring)
                {
                    ...
                    if(tmp2)ans.push_back('1'+i);

                    InputString = InputString.substr(MaxMatchedSubstring);
                    break;
                }
            }
        }else
        {
            ...
            int tmp3 = tmp2;
            tmp3++;
            if(tmp2)ans.push_back(InputString[0]);
...
int main()
{
    ...
    getline(in, regexList_cnt_str);

    int regexList_cnt_int;
    
    // cout<<regex_cnt_int;
    regexList_cnt_int  = stoi(regexList_cnt_str);
    ...
    vector<string> regexListarr;
     

     int tmp_int2 = 1;

    for(int i=0;i<regexList_cnt_int && tmp_int2;i++)
    {
        //string regex
        string regex;

        // reading  a line from input.txt         
        getline(in,regex);

        //pushing back into regexListarr
        regexListarr.push_back(regex);
    }
    ...
    if(tmp_int1)ans = printResult(InputString, tmp_int1, regexListarr, regexList_cnt_int);
